			...
			{session?.user ? (
				<div className='flex items-center justify-end md:order-2'>
					...
					<div className='flex items-center'>
						<button
							className='hidden md:block py-2 px-4 bg-gray-50 text-gray-700 hover:bg-gray-100 rounded-sm cursor-pointer'
							onClick={() => signOut()}>
							Sign Out
						</button>
					</div>
				...
				<ul className='flex flex-col font-semibold text-sm uppercase p-4  mt-4 rounded-sm  bg-gray-50 md:flex-row md:items-center md:space-x-10 md:p-0 md:mt-0 md:border-0 md:bg-white '>
					...
					{session ? (
						<li className='pt-2 md:pt-0'>
							<button
								onClick={() => signOut()}
								className='md:hidden block py-2.5 px-4 bg-red-400 text-white hover:bg-red-600 rounded-sm cursor-pointer'>
								Sign Out
							</button>
						</li>
					) : (
						<li className='pt-2 md:pt-0'>
							<Link
								href='/signin'
								className='block py-2.5 px-6 bg-orange-400 text-white hover:bg-orange-500 rounded-sm'>
								Sign In
							</Link>
						</li>
					)}
